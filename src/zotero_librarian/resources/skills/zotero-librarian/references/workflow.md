...
zotero-librarian --json plan validate edits.jsonl --taxonomy taxonomy.toml --library-json library.json
...
```bash
zotero-librarian --json plan preview metadata-edits.jsonl --extended
```
...
```bash
zotero-librarian --json plan apply sample.jsonl --yes
...
```bash
zotero-librarian --json plan apply metadata-sample.jsonl --extended --yes
```
...
```bash
zotero-librarian --json library export --out library.json
zotero-librarian --json library audit library.json --expect-items <BASELINE>
...
```bash
zotero-librarian --json identity audit library.json --only-conflicts --workers 4
zotero-librarian --json identity audit library.json --keys <ITEM_KEY> --only-conflicts
```
...
zotero-librarian identity report library.json identity-report.json \
...
```bash
zotero-librarian identity decision library.json identity-report.json \
...
```bash
zotero-librarian --json identity plan library.json identity-report.json \
...
zotero-librarian --json plan preview source-identity-plan.jsonl --extended
```
...
zotero-librarian --json metadata enrich missing.json --output abstracts.jsonl --report report.json
zotero-librarian --json plan validate abstracts.jsonl --taxonomy taxonomy.toml
zotero-librarian --json plan preview abstracts.jsonl
```
...
zotero-librarian --json metadata enrich missing-doi.json --field doi \
